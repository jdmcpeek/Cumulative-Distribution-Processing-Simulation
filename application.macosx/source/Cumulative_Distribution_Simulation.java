...
public class Cumulative_Distribution_Simulation extends PApplet {
...
public float ScaleY(float y) {
    ...
    return y * (length_y / 0.5f); 
};

public float normal_dist(float x, float variance, float mean) {
...
public ArrayList<float[]> createCurve() {
...
public float AreaTrapezoid(float s1, float s2) {
    int height = 1;  // just so you know... we're doing it in pixels
    return height * ((s1 + s2) / 2);
};


public ArrayList<Float> ApproximateIntegral(float bound, int total_length) {
...
public void draw() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--full-screen", "--bgcolor=#666666", "--stop-color=#cccccc", "Cumulative_Distribution_Simulation" };
    if (passedArgs != null) {
